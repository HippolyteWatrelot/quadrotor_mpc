...
Eigen::Vector3d linear_acados_MPC::quaternion_to_euler(Eigen::Vector4d& q) const
{
...
void linear_acados_MPC::euler_callback(const geometry_msgs::PoseStamped::ConstPtr& pose)
{
    current_ori(0) = pose->pose.orientation.w;
    current_ori(1) = pose->pose.orientation.x;
    current_ori(2) = pose->pose.orientation.y;
...
{   
    ...
    euler_subscriber   = _nh.subscribe<geometry_msgs::PoseStamped>("/ground_truth_to_tf/pose", 1, &linear_acados_MPC::euler_callback, this);
...
void linear_acados_MPC::init_x0_u0(bool init_command)
{
    ...
    x0[7] = current_vel(2);
    x0[8] = current_acc(2);
    x0[9] = current_euler(2);
    x0[10] = current_euler_vel(2);
    x0[11] = current_euler_acc(2);
