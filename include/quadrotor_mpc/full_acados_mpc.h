#ifndef FULL_ACADOS_MPC_H_INCLUDED
#define FULL_ACADOS_MPC_H_INCLUDED
...
extern "C" {
    #include "acados_solver/full/acados_solver_full_drone_dynamics.h"
    #include "acados_solver/full/full_drone_dynamics_model/full_drone_dynamics_model.h"
}



class full_acados_MPC
{

public:

    full_acados_MPC(bool gt, bool w);
    ~full_acados_MPC();
    ...
    Eigen::Vector3d forward_body_twist_command_w(double state[], double command[]);
    void build_solver();
    void iteration_test();
...
private:
    full_drone_dynamics_solver_capsule *capsule;    // struct
    int status;
    bool ground_truth;
    bool wind;
    std::string method;
    int trajL=0; 
    int N;
    static const int nx=26;
    static const int state_nx=20; 
...
#endif // FULL_ACADOS_MPC_H_INCLUDED
